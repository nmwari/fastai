...
# if [ ! -f ~/.ssh/$keyname.pem ]
# then
# 	aws ec2 create-key-pair --key-name $keyname --query 'KeyMaterial' --output text > ~/.ssh/$keyname.pem
# 	chmod 400 ~/.ssh/$keyname.pem
# fi

export instanceId=$(aws ec2 run-instances --image-id $ami --count 1 --instance-type $instanceType --key-name $keyname --security-group-ids $securityGroupId --subnet-id $subnetId --associate-public-ip-address --block-device-mapping "[ { \"DeviceName\": \"/dev/sda1\", \"Ebs\": { \"VolumeSize\": 128, \"VolumeType\": \"gp2\" } } ]" --instance-market-options "{ \"MarketType\": \"spot\", \"SpotOptions\": { \"MaxPrice\": \"0.90\", \"SpotInstanceType\": \"persistent\", \"InstanceInterruptionBehavior\": \"hibernate\"}}" --query 'Instances[0].InstanceId' --user-data file://fast_ai_setup.sh  --output text)
...
echo ssh -i ~/.ssh/$keyname.pem ubuntu@$instanceUrl >> $name-commands.txt
...
echo Connect to your instance: ssh -i ~/.ssh/$keyname.pem ubuntu@$instanceUrl
